...
func Test_kDB_Backup(t *testing.T) {
	path := dbPath
	db, err := Reopen(path)
	if err != nil {
	...
	err = db.Backup("/tmp/kdb/backup-db0")
	if err != nil {
		t.Error(err)
	}
}
...
func Test_kDB_Close(t *testing.T) {
	db := InitDb()
	defer db.Close()
}

func Test_kDB_Sync(t *testing.T) {
	db := InitDb()
	defer db.Close()
...
func Test_kDB_Reclaim(t *testing.T) {
	config := DefaultConfig()
	config.DirPath = "/tmp/kdb/db-reclaim"
	config.IdxMode = KeyOnlyRamMode
	config.RwMethod = storage.FileIO
	config.BlockSize = 4 * 1024 * 1024
...
func writeMultiLargeData(db *kDB) {
	keyPrefix := "test_key_"
